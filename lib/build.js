...
const genEnv = ()=>{
    const config = {
        AppID: process.env.AZURE_APPID,
        AppSecret: process.env.AZURE_APPSECRET,
        redirect: process.env.AZURE_REDIRECTURL
    }
    fs.writeJSONSync(path.join('./dist','config.json'),config,{spaces:4})
}
